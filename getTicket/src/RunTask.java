...
public class RunTask extends Thread {
	...
	public static void getData() {
		...
		if (!timeLine.equals(""))
			intTime = Long.parseLong(timeLine) * 60 * 1000;// 返回毫秒间隔
		train = TicketMonitor.targetTrainNum;// 监测车次
		for (int i = 0; i < 10; i++)
			selectedType[i] = TicketMonitor.selectedType[i];
	}
	...
	public static void testMail() {
		...
		try {
			...
			if (Mail.send(smtp, from, to, "测试", content, from, password)) {
				...
				JOptionPane.showMessageDialog(null, "邮件发送失败！请检查网络及smtp设置！");
		} catch (Exception e8) {
	...
	public void run() {
		...
		while (true) {
			...
			try {
				...
				/*
				 * if(flag2==0) { content="不好意思，当前时间本车次所有座位类型均无票\n，谢谢您使用本系统";
				 * 
				 * }
				 */
				if (flag2 == 1) {
					content = content + "您的查询结果已给出，谢谢您使用本系统";
					if (Mail.send(smtp, from, to, subject, content, from, password)) {
						TicketMonitor.bt3.setSelected(false);
				...
				/*
				 * if(flag==1) { for(int i=0;i<10;i++) { try{ content=content+
				 * " "+Test.getTrainInfo(train,ticketType[i],result);
	...
	public static void search() {
		...
		try {
			...
			// ArrayList<String> imgSrc = RegexString(result,"img
			// src=\"([^\"]*)");//"href=\"[^\"]?");//"img src=\"(.+?)\"");
			// getTrainList
