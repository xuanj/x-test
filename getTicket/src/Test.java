...
public class Test {
	...
	public static String getTrainInfo(String train_num, String type, String data) {// type为需求的票的类型
																					// ticket_type=["swz_num","tz_num","zy_num","ze_num","gr_num","rw_num","yw_num","rz_num","yz_num","wz_num"]
																					// #swz商务座
																					// zy一等座
																					// ze二等座
																					// wz无座
	...
	/*
	 * public static void main(String[] args) { // 定义即将访问的链接 // String url =
	 * "http://blog.sina.com.cn/s/blog_5f99653f01016yr8.html"; // 访问链接并获取页面内容 //
	 * String result = SendGet(url); // 使用正则匹配图片的src内容 // ArrayList<String>
	 * imgSrc = RegexString(result,"img src=\"([^\"]*)");//"href=\"[^\"]?");/
	 * /"img src=\"(.+?)\""); // 打印结果 //updateStationList();
	 * //getStationName("北京北"); //System.out.println(imgSrc); //TicketMonitor
	 * mainWindow = new TicketMonitor(); //mainWindow.setVisible(true); //
	 * updateStationList(); Test te=new Test();//创建一个对象te
	 * te.setUI();//调用setUI()函数，显示UI； //String
	 * date=dc.getDate().toInstant().toString().substring(0,10);
	 * //te.RunTask();//如果调用该函数出现空指针异常 bt1.addMouseListener(new MouseAdapter() {
	 * public void mousePressed(MouseEvent e) { // ***** 检查各个参数情况，调用相应接口查询 *****
	 * String type1 = null;//乘客类型 if(rb1.isSelected()) type1="ADULT";
	 * if(rb2.isSelected()) type1="0X00"; String fromSta=tf1.getText();//出发站
	 * String toSta=tf2.getText();//终止站 String
	 * date=dc.getDate().toInstant().toString().substring(0,10);//获得日期 String
	 * url=finishSearchUrl(fromSta,toSta,date,type1); System.out.println(url);
	 * String result = SendGet(url); ArrayList<String> s2=getTrainList(result);
	 * //System.out.println(s2); // ArrayList<String> imgSrc =
	 * RegexString(result,"img src=\"([^\"]*)");//"href=\"[^\"]?");/
	 * /"img src=\"(.+?)\""); // getTrainList String str="";//str保存输出内容
	 * for(String s:s2){//遍历s2字符串列表保存的车次序列 String s0=s; s0=s0+'\t';
	 ...
	 * 
	 * }catch(Exception e6){ System.out.println(e6); }
	 * try{s0=s0+" "+getTrainInfo(s,"yw_num",result);
	 * 
	 * }catch(Exception e7){ System.out.println(e7); }
	 * try{s0=s0+" "+getTrainInfo(s,"rz_num",result);
	 * 
	 * }catch(Exception e8){ System.out.println(e8); }
	 * try{s0=s0+" "+getTrainInfo(s,"yz_num",result);
	 * 
	 * }catch(Exception e9){ System.out.println(e9); }
	 * try{s0=s0+" "+getTrainInfo(s,"wz_num",result);
	 * 
	 * }catch(Exception e10) { System.out.println(e10); } str+=s0+'\n'; }
	 * TextField.setText(str);//在TestField显示字符串 } }); //type为需求的票的类型
	 * ticket_type=["swz_num","tz_num","zy_num","ze_num","gr_num","rw_num",
	 * "yw_num","rz_num","yz_num","wz_num"] #swz商务座 zy一等座 ze二等座 wz无座 yz硬座 rw软卧
	 * yw硬卧 // #tz特等座 gr高级软卧 yz硬座 /*String smtp = mPanel1.getText(); //smtp地址
	 * String from = mPanel2.getText(); //发件人 String to = mPanel4.getText();
	 * //收件人 String copyto = null; String subject = "邮件主题"; //邮件主题 String
	 * content=""; //邮件内容 String username="用户名"; String
	 * password=mPanel3.getText(); //密码 String filename = "info.txt"; //附件
	 * String timeLine=mPanel5.getText();//监测间隔 long
	 * intTime=Long.parseLong(timeLine)*60*1000; String
	 * train=mPanel6.getText();//监测车次
	 * 
	 * try{ Mail.sendAndCc(smtp, from, to, copyto, subject, content, username,
	 * password, filename); }catch(Exception e8) { System.out.println(e8); }
	 */

	/*
	 * bt5.addMouseListener(new MouseAdapter() {//感觉这个函数按钮没有用。。。
	 * 
	 * @Override public void mousePressed(MouseEvent e) { // ***** SMTP测试相关接口调用
	 * *****
	 * 
	 * } }); bt6.addMouseListener(new MouseAdapter() {//重置监测内容
	 * 
	 * @Override public void mousePressed(MouseEvent e) { mtf1.setText("");
	 * mtf2.setText(""); mtf3.setText(""); mtf4.setText(""); mtf5.setText("");
	 * mtf6.setText(""); } }); }
	 * 
